from mcproto import MinecraftClient
from mcproto.packets import ChatMessagePacket, PlayerPositionPacket
...
async def movement_loop(data, client):
    while client.connected:
        try:
            ...
            if action == "walk":
                ...
                await client.send_packet(PlayerPositionPacket(x=x, y=0, z=z))
            elif action == "jump":
                await client.send_packet(PlayerPositionPacket(x=0, y=1, z=0))
            await asyncio.sleep(2)
        except Exception as e:
            print(f"Movement error: {str(e)}")
async def chat_loop(data, client):
    while client.connected:
        try:
            message = _infer_markov_chat("hii guys")
            if message:
                await client.send_packet(ChatMessagePacket(message=message))
